...
`/prettfy` is a Pro preview command for administrators with Manage Channels and Manage Roles. The bot collects the design brief in DMs, proposes channel names first, and changes only explicitly approved names. Every channel ID is accounted for, including forum and media channels; large servers are generated in safe batches. Non-voice channel names are normalised to lowercase, spaces become hyphens, ampersands become `and`, and decorative brackets use the supported `『example』` style. Channel changes open in a temporary HTML review that lists changed and unchanged channels, then is deleted when the user answers or after one hour. Roles retain the DM preview flow. A live checklist uses ✅ for complete, 🔁 for in progress, and ❎ for not completed. OpenRouter responses are validated and retried once when malformed or incomplete. Prettfy never changes permission overwrites, role permissions, hierarchy, colours, topics, assignments, or channel types. Use `/prettfy undo_last:true` to preview and restore names from the latest active run.
