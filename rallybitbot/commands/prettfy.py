...
def _response_content(response: dict[str, Any]) -> str:
    try:
        ...
        raise PrettfyError("OpenRouter returned an incomplete response. Try again in a moment.") from exc
    if isinstance(content, str):
        return content
    if isinstance(content, list):
        return "".join(str(part.get("text", "")) for part in content if isinstance(part, dict))
    raise PrettfyError("OpenRouter returned an unsupported response format.")
...
def request_plan(
    *,
    ...
    try:
        ...
        raise PrettfyError("OpenRouter returned an unreadable response. No Discord changes were made.") from exc
    try:
        result = json.loads(_response_content(response))
    except json.JSONDecodeError as exc:
        raise PrettfyError("OpenRouter did not return a valid naming plan. No Discord changes were made.") from exc
    if not isinstance(result, dict):
        raise PrettfyError("OpenRouter returned an invalid naming plan. No Discord changes were made.")
    return result
...
async def _generate_approved_plan(
    ...
    *,
    ...
    for revision in range(MAX_REVISIONS):
        await user.send(f"Creating the {label.lower()} preview…")
...
async def _undo(client: discord.Client, guild: discord.Guild, user: discord.abc.User) -> None:
...
async def _wizard(client: discord.Client, guild: discord.Guild, user: discord.abc.User, undo_last: bool) -> None:
    try:
        ...
        channel_result = await _generate_approved_plan(
            client, user, label="Channels", inventory=channel_inventory, brief=brief, style=style
        )
        ...
        channel_count, channel_failures = await _apply_channels(guild, user, channel_plan, record_id)
        ...
        role_result = await _generate_approved_plan(
            client, user, label="Roles", inventory=role_inventory, brief=brief, style=f"{style}; roles: {role_style}"
        )
        role_count = 0
        ...
        if role_result is not None:
            ...
            role_count, role_failures = await _apply_roles(guild, user, role_plan, record_id)
        else:
